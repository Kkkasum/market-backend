...
class MarketRepo:
    ...
    async def get_numbers() -> list[MarketNumber] | None:
        async with new_session() as session:
            query = (
                select(MarketNumber)
                .options(joinedload(MarketNumber.user_number))
            )
    ...
    async def get_usernames() -> list[MarketUsername] | None:
        async with new_session() as session:
            query = (
                select(MarketUsername)
                .options(joinedload(MarketUsername.user_username))
            )
            res = (await session.execute(query)).scalars().all()
